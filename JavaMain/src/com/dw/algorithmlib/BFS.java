...
public class BFS {
	public void bfsVisit(int n, int[][] connections) {
	...
	private void bfs(int src, HashMap<Integer, List<Integer>> graph, boolean[] visited) {
		...
		while(!queue.isEmpty()) {
			...
			System.out.println("Point " + node + " is dequeued");
			if(n==null) continue;
			for(int m : n) {
				if(!visited[m]) {
					visited[m] = true;
					queue.offer(m);
					System.out.println("Point " + m + " is visited");
				}
			}
		}
	}

	public void connect(HashMap<Integer, List<Integer>> graph, Integer v1, Integer v2) {
