...
/**
 * Property editor for Maps, converting any source Map to a given target Map
 * type.
 *
...
public class CustomMapEditor extends PropertyEditorSupport {
	 ...
	 * different Map type or an array, it will be converted to a default
	 * implementation of the given Map type. If the value is anything else, a target
	 * Map with that single value will be created.
	 * <p>
	 * The default Map implementations are: TreeMap for SortedMap, and LinkedHashMap
	 * for Map.
	 * 
	 * @param mapType        the target type, which needs to be a sub-interface of
	 *                       Map or a concrete Map class
	 * @param nullAsEmptyMap ap whether to convert an incoming {@code null} value to
	 *                       an empty Map (of the appropriate type)
	 * @see java.util.Map
	 * @see java.util.TreeMap
	 * @see java.util.LinkedHashMap
	 */
	...
	/**
	 * Create a Map of the given type, with the given initial capacity (if supported
	 * by the Map type).
	 * 
	...
	/**
	 * Return whether to always create a new Map, even if the type of the passed-in
	 * Map already matches.
	 * <p>
	 * Default is "false"; can be overridden to enforce creation of a new Map, for
	...
	/**
	 * Hook to convert each encountered Map key. The default implementation simply
	 * returns the passed-in key as-is.
	 * <p>
	 * Can be overridden to perform conversion of certain keys, for example from
	 * String to Integer.
	 * <p>
	 * Only called if actually creating a new Map! This is by default not the case
	 * if the type of the passed-in Map already matches. Override
	 * {@link #alwaysCreateNewMap()} to enforce creating a new Map in every case.
	 * 
	 ...
	 * <p>
	 * Can be overridden to perform conversion of certain values, for example from
	 * String to Integer.
	 * <p>
	 * Only called if actually creating a new Map! This is by default not the case
	 * if the type of the passed-in Map already matches. Override
