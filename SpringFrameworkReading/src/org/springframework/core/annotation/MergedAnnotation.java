...
public interface MergedAnnotation<A extends Annotation> {
	 ...
	 * {@linkplain #isMetaPresent() meta-present} annotations within the context of
	 * the {@link SearchStrategy} used.
	 * 
	 ...
	 * present annotation is one that the user has explicitly declared and not one
	 * that is {@linkplain #isMetaPresent() meta-present} or
	 ...
	 * annotation is an annotation that the user hasn't explicitly declared, but has
	 * been used as a meta-annotation somewhere in the annotation hierarchy.
	 * 
	 ...
	 * A directly declared annotation has a distance of {@code 0}, a meta-annotation
	 * has a distance of {@code 1}, a meta-annotation on a meta-annotation has a
	 * distance of {@code 2}, etc. A {@linkplain #missing() missing} annotation will
	 * always return a distance of {@code -1}.
	 * 
	...
	/**
	 * Get the index of the aggregate collection containing this annotation. Can be
	 * used to reorder a stream of annotations, for example, to give a higher
	...
	/**
	 * Get the source that ultimately declared the root annotation, or {@code null}
	 * if the source is not known. If this merged annotation was created
	 * {@link MergedAnnotations#from(java.lang.reflect.AnnotatedElement) from} an
	 * {@link AnnotatedElement} then this source will be an element of the same
	 * type. If the annotation was loaded without using reflection, the source can
	 * be of any type, but should have a sensible {@code toString()}.
	 * Meta-annotations will always return the same source as the {@link #getRoot()
	 * root}.
	 * 
	...
	/**
	 * Get the source of the meta-annotation, or {@code null} if the annotation is
	 * not {@linkplain #isMetaPresent() meta-present}. The meta-source is the
	 * annotation that was meta-annotated with this annotation.
	 * 
	 ...
	 * {@link #getRoot() root}. Provides a useful way to uniquely identify a merged
	 * annotation instance.
	 * 
	 ...
	 * 
	 * @param attributeName the attribute name
	 * @param type          the attribute type. Must be compatible with the
	 *                      underlying attribute type or {@code Object.class}.
	 * @return an optional value or {@link Optional#empty()} if there is no matching
	 *         attribute
	 */
	 ...
	 * 
	 * @param attributeName the attribute name
	 * @return an optional of the default value or {@link Optional#empty()} if there
	 *         is no matching attribute or no defined default
	 */
	 ...
	 * rules applied. Aliases to {@link #getMetaSource() meta-source} attributes
	 * will not be applied.
	 * 
	 ...
	 * annotation.
	 * <p>
	 * The {@link Adapt adaptations} may be used to change the way that values are
	 * added.
	 * 
	 * @param adaptations adaptations that should be applied to the annotation
	 *                    values
	...
	/**
	 * Create a type-safe synthesized version of this annotation that can be used
	 * directly in code.
	 * <p>
	 * The result is synthesized using a JDK {@link Proxy} and as a result may incur
	 * a computational cost when first invoked.
	 * 
	...
	/**
	 * Optionally create a type-safe synthesized version of this annotation based on
	 ...
	 * @return a optional containing the synthesized version of the annotation or an
	...
	/**
	 * Create a {@link MergedAnnotation} that represents a missing annotation (i.e.
	 ...
	 *                   information and logging. It does not need to
	 *                   <em>actually</em> contain the specified annotations, and it
	 ...
	 * type. The resulting annotation will not have any attribute values but may
	 * still be used to query default values.
	 * 
	...
	/**
	 * Create a new {@link MergedAnnotation} instance of the specified annotation
	 * type with attributes values supplied by a map.
	 * 
	 * @param annotationType the annotation type
	 ...
	 * type with attributes values supplied by a map.
	 * 
	 * @param source         the source for the annotation. This source is used only
