...
public abstract class MergedAnnotationPredicates {
	 ...
	 * will match the first annotation, and any subsequent run that have the same
	 * distance.
	 * <p>
	 * NOTE: This predicate only matches the first run. Once the extracted value
	 * changes, the predicate always returns {@code false}. I.e. if you have a set
	 * of annotations with distances {@code [1, 1, 2, 1]} then only the first two
	 * will match.
	 * 
	...
	/**
	 * Create a new stateful, single use {@link Predicate} that matches annotations
	 * that are unique based on the extracted key. For example
