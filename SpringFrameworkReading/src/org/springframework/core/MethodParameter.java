...
public class MethodParameter {
	...
	/**
	 * Return a variant of this {@code MethodParameter} with the type for the
	 * current level set to the specified value.
	 * 
	 ...
	 * parameter but one nesting level deeper.
	 * 
	 * @since 4.3
	 */
	 ...
	 * either in the form of Java 8's {@link java.util.Optional}, any variant of a
	 * parameter-level {@code Nullable} annotation (such as from JSR-305 or the
	 * FindBugs set of annotations), or a language-level nullable type declaration
	 * in Kotlin.
	 * 
	 * @since 4.3
	 */
	 ...
	 * containing class.
	 * 
	 * @param containingClass a specific containing class (potentially a subclass of
	 *                        the declaring class, e.g. substituting a type
	 *                        variable)
	 * @since 5.2
	...
	private static class KotlinDelegate {
		 ...
		 * Kotlin type or an optional parameter (with a default value in the Kotlin
		 * declaration).
		 */
		public static boolean isOptional(MethodParameter param)
		{
			...
			if (method != null && index == -1) {
				...
				if (function != null) {
					...
					KParameter parameter = parameters.stream().filter(predicate).collect(Collectors.toList())
							.get(index);
		...
		/**
		 * Return the return type of the method, with support of suspending functions
